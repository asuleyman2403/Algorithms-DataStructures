...
int r[550];
int p[550];
double getd(pair<int,int> x, pair<int,int> y)
{
...
int main()
{
	...
	for(int it=0;it<t;it++){
		...
		for(int i=0;i<n;i++){
			for(int j=i+1;j<n;j++){
				g.push_back(make_pair(getd(point[i],point[j]),make_pair(i, j)));
			}
		}
		...
		{
			dist.push_back(getd(point[result[i].first], point[result[i].second]));
		}
